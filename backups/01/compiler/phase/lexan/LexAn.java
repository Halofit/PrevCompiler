...
public class LexAn extends Phase {
	...
	*/
		...
		try {
			...
			while (true) {
				...
				if (curr == -1) {
					...
					if((curr & 0x80) != 0) {
						throw new CompilerError( getPosition() + " Non-ascii character with code: <"  + curr + "> !");
					}
				...
				switch (state) {
					case NONE:
						switch (id) {
							...
							case ERROR:
							default:
								throw new CompilerError(getPosition() + " Unknown character: " + c);
						}
						break;
					case COMM:
						switch (id) {
					...
					case WORD:
						...
						break;
					default:
						throw new CompilerError(getPosition() + " Lexer: Unreachable statement reached");
				}
				...
				if (endFlag == EndFlag.END) {
					...
					throw new CompilerError(getPosition() + " Unknown EndFlag!");
				}
			} // the read char loop

			log(sym);
		} catch (IOException e) {
			throw new CompilerError(getPosition() + " Error reading character!");
		}

		if (sym.token == Symbol.Token.CONST_CHAR) {
			if (!isValidCharConst(sym.lexeme)) {
				throw new CompilerError(getPosition() + " Invalid char constant <" + sym.lexeme + ">");
			}
		}

		if (sym.token == Symbol.Token.CONST_STRING) {
			if (!isValidStringConst(sym.lexeme)) {
				throw new CompilerError(getPosition()
	...
	private Position getPosition() {
		return new Position(
				this.task.srcFName, this.prevLine, this.prevColumn+1,
		...
		switch (s) {
			...
			case SYMBOL:
				switch (lexeme) {
					...
					default:
						throw new CompilerError(getPosition() + " Unknown symbol <" + lexeme + ">");
				}
			case EOF:
			...
			case WS:
			case COMM:
				throw new CompilerError(getPosition() + " idToken: Whitspace should never reach idToken!");
		}
		throw new CompilerError(getPosition() + " Lexer: idToken: unknown token to id");
	}
